...
五极云是一款企业级 AI \+ aPaaS 零代码快速开发平台，基于前后端分离架构，可通过可视化拖拽快速搭建 CRM、进销存、OA 办公、流程审批、数据大屏、仓储管理等全场景企业业务系统。
...
本项目为前后端分离架构，开源仓库完整包含后端、前端、小程序、数据库脚本及配套文档，结构清晰，支持源码部署与Docker容器化部署两种方式。
...
- **backend**：SpringBoot Java后端服务，承载项目核心业务逻辑、接口服务、权限控制、流程引擎等核心能力

- **frontend**：基于React开发的PC端管理后台，包含零代码可视化设计、业务操作、数据展示等页面

- **miniprogram**：微信小程序端源码，支持移动端查看、审批、操作业务数据，实现多端协同

- **sql**：项目数据库初始化脚本，包含数据表结构、基础配置数据、初始权限账号等，部署必备

- **docs**：项目配套文档，包含部署教程、接口文档、使用手册、常见问题等辅助资料

### 2\.3 源码手动部署（开发/二次开发使用）

该部署方式适用于开发者二次开发、本地调试、自定义改造场景，需手动配置服务器环境。
...
支持全平台自动安装 Docker、Docker Compose、初始化数据库、启动全套服务，内置文件 SHA256 哈希校验，防止文件篡改，部署安全可靠、极简高效，适合生产环境快速部署。
