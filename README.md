# 五极云
🔥 AI + aPaaS 企业级零代码快速开发平台｜私有化部署｜Docker一键部署

> **五极云** 基于 Java + React 前后端分离架构，开源零代码平台，面向企业、工厂、政企，可视化拖拽快速搭建内部业务系统，支持内网私有化部署，数据本地留存。
> 开箱即用，无需大量编码，快速实现 OA、CRM、进销存、流程审批、数据台账、业务管理系统。

## ✨ 核心亮点
- ✅ **私有化部署**：全部数据存储本地服务器，不上传第三方云端，满足内网安全、等保合规场景
- ✅ **Docker一键部署**：一条命令完成整套环境安装，自动处理 MySQL / Redis / MongoDB，降低部署成本
- ✅ **零代码可视化开发**：表单、页面、菜单、视图拖拽搭建，业务人员也可以开发业务系统
- ✅ **内置工作流引擎**：支持串行、并行、条件分支审批，适配企业各类审批流程
- ✅ **细粒度权限管控**：组织架构、角色、数据权限、按钮权限完整体系
- ✅ **多端支持**：PC后台 + 微信小程序，手机端处理审批、查看业务数据
- ✅ **源码开放**：社区版开源，支持二次开发，Java开发者可深度定制业务逻辑

## 🚀 快速开始｜一键部署
>支持 CentOS / Ubuntu / Debian Linux 系统

```bash
# CentOS / 腾讯云OpenCloud
curl -fsSL https://cloud.elemost.com/docker/centos/install.sh | bash

# Ubuntu / 阿里云AlibabaCloud
...
💡提示：一键脚本自动安装 Docker、数据库、中间件；如果服务器已经安装 Docker，脚本会自动跳过 Docker 安装步骤。
...
- **开源社区版**：免费开源，用于学习、企业内部非商用；提供全部基础零代码能力，私有化部署。

- **标准版 / 专业版 / 旗舰版**：商业授权版本，提供高级功能、技术支持、实施服务、信创适配、7×24 服务。
